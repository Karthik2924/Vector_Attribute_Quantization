...
def _test():
    ...
    for noise in np.linspace(0.0, 1.0, 11):
        latents = sources + np.normal(0, noise, sources.shape)
    ...
    print(f'noise=inf : explicitness_reg={explicitness_regression(np.zeros(sources.shape), sources)}, explicitness_clf={explicitness_classification(np.zeros(sources.shape), sources)}')
